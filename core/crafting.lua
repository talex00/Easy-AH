...
local search_tab = require 'EasyAH.tabs.search'

function EasyAH.handle.LOAD()
    if not EasyAH.account_data.crafting_cost then
        return
    end
    EasyAH.event_listener('ADDON_LOADED', function()
...
do
    ...
    function craft_ui_loaded()
        EasyAH.hook('CraftFrame_SetSelection', T.vararg-function(arg)
            local ret = T.temp-T.list(EasyAH.orig.CraftFrame_SetSelection(unpack(arg)))
            ...
            for i = 1, GetCraftNumReagents(id) do
                ...
                local count = EasyAH.select(3, GetCraftReagentInfo(id, i))
    ...
    function trade_skill_ui_loaded()
        EasyAH.hook('TradeSkillFrame_SetSelection', T.vararg-function(arg)
            local ret = T.temp-T.list(EasyAH.orig.TradeSkillFrame_SetSelection(unpack(arg)))
            ...
            for i = 1, GetTradeSkillNumReagents(id) do
                ...
                local item_id, suffix_id = info.parse_link(link)
                local count = EasyAH.select(3, GetTradeSkillReagentInfo(id, i))
                local _, price, limited = info.merchant_info(item_id)
                local value = price and not limited and price or history.value(item_id .. ':' .. suffix_id)
                if not value then
                    total_cost = nil
