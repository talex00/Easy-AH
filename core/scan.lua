...
do
	...
	function M.start(params)
		...
		-- New scan generation: lets history reset each item's market floor from
		-- the current scan instead of a stale earlier-today minimum.
		history.begin_scan()
		do (params.on_scan_start or pass)() end
		local thread_id = EasyAH.thread(scan)
	...
	function get_state()
		for _, state in scan_states do
			if state.id == EasyAH.thread_id() then
				return state
			end
		end
	end
...
function scan()
	...
	if get_query() and not get_state().stopped then
		...
		if get_query().blizzard_query then
			if (get_query().blizzard_query.first_page or 0) <= (get_query().blizzard_query.last_page or EasyAH.huge) then
...
do
	...
	function submit_query()
		...
		if get_state().params.type ~= 'list' then
			...
			return EasyAH.when(CanSendAuctionQuery, submit)
		end
	end
end
...
function scan_page(i)
	...
	if auction_info and (auction_info.owner or get_state().params.ignore_owner or EasyAH.account_data.ignore_owner) then
		...
		if (get_state().params.auto_buy_validator or pass)(auction_info) and auction_info.buyout_price >0 and auction_info.owner ~= UnitName("player") then
			...
			EasyAH.when(signal_received, scan_page, i)
			return EasyAH.place_bid(auction_info.query_type, auction_info.index, auction_info.buyout_price, send_signal)
		elseif (get_state().params.auto_bid_validator or pass)(auction_info) and auction_info.owner ~= UnitName("player") and auction_info.high_bidder == nil then
			local send_signal, signal_received = EasyAH.signal()
			EasyAH.when(signal_received, scan_page, i)
			return EasyAH.place_bid(auction_info.query_type, auction_info.index, auction_info.bid_price, send_signal)
...
function wait_for_results()
    if get_state().params.type == 'bidder' then
        return EasyAH.when(function() return EasyAH.bids_loaded() end, accept_results)
    elseif get_state().params.type == 'owner' then
...
function wait_for_owner_results()
    if get_state().page == EasyAH.current_owner_page() then
	    return accept_results()
    else
	    local updated
        EasyAH.on_next_event('AUCTION_OWNED_LIST_UPDATE', function() updated = true end)
	    return EasyAH.when(function() return updated end, accept_results)
    end
end

function wait_for_list_results()
    local updated, last_update
    local listener_id = EasyAH.event_listener('AUCTION_ITEM_LIST_UPDATE', function()
        last_update = GetTime()
        updated = true
    end)
    local timeout = EasyAH.later(5, get_state().last_list_query)
    local ignore_owner = get_state().params.ignore_owner or EasyAH.account_data.ignore_owner
	return EasyAH.when(function()
		...
		EasyAH.kill_listener(listener_id)
		if not last_update and timeout() then
			return submit_query()
		else
			return accept_results()
