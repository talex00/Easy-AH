module 'EasyAH.gui.auction_listing'
...
local tooltip = require 'EasyAH.core.tooltip'
...
local TIME_LEFT_STRINGS = {
	EasyAH.color.red'30m', -- Short
	EasyAH.color.orange'2h', -- Medium
	EasyAH.color.yellow'8h', -- Long
	EasyAH.color.blue'24h', -- Very Long
}
...
M.search_columns = {
    ...
    },
    {
        ...
        fill = function(cell, record)
            ...
            display_level = UnitLevel'player' < record.level and EasyAH.color.red(display_level) or display_level
    ...
    },
    {
        ...
        fill = function(cell, record, count, own, expandable)
            local numAuctionsText = expandable and EasyAH.color.link(count) or count
            if own > 0 then
                numAuctionsText = numAuctionsText .. (' ' .. EasyAH.color.yellow('(' .. own .. ')'))
            end
...
--            end
    ...
    },
    {
        ...
        align = 'CENTER',
        fill = function(cell, record)
            cell.text:SetText(info.is_player(record.owner) and (EasyAH.color.yellow(record.owner)) or (record.owner or '?'))
        end,
        cmp = function(record_a, record_b, desc)
    ...
    },
    {
        ...
        fill = function(cell, record)
            ...
            if record.high_bidder then
	            price_color = EasyAH.color.green
            elseif record.high_bid ~= 0 then
	            price_color = EasyAH.color.orange
            end
            local price
            if record.high_bidder then
    ...
    },
    {
        ...
        cmp = function(record_a, record_b, desc)
            ...
            price_a = price_a > 0 and price_a or (desc and -EasyAH.huge or EasyAH.huge)
            price_b = price_b > 0 and price_b or (desc and -EasyAH.huge or EasyAH.huge)
    ...
    },
    {
        ...
        cmp = function(record_a, record_b, desc)
            local pct_a = record_percentage(record_a) or (desc and -EasyAH.huge or EasyAH.huge)
            local pct_b = record_percentage(record_b) or (desc and -EasyAH.huge or EasyAH.huge)
...
M.auctions_columns = {
    ...
    },
    {
        ...
        align = 'CENTER',
        fill = function(cell, record)
            cell.text:SetText(record.high_bidder or EasyAH.color.red 'No Bids')
        end,
        cmp = function(record_a, record_b, desc)
...
M.bids_columns = {
    ...
    },
    {
        ...
        fill = function(cell, record)
            local status
            if record.high_bidder then
                status = EasyAH.color.yellow'High Bidder'
            else
                status = EasyAH.color.red'Outbid'
            end
...
function record_percentage(record)
    ...
    if historical_value > 0 then
        if record.unit_buyout_price > 0 then
            return EasyAH.round(100 * record.unit_buyout_price / historical_value)
        end
        return nil, EasyAH.round(100 * record.unit_bid_price / historical_value)
    end
end
...
local methods = {
    ...
    UpdateRowInfo = function(self)
	    ...
	    local single_item = EasyAH.all(records, function(record) return record.item_key == records[1].item_key end)
    ...
    UpdateRows = function(self)
        ...
        for _, cell in self.headCells do
            ...
            tex:SetTexture[[Interface\AddOns\EasyAH-AddOn\WorldStateFinalScore-Highlight]]
    ...
    RemoveAuctionRecord = function(self, record)
        local index = EasyAH.key(self.records, record)
        if index then
    ...
    ContainsRecord = function(self, record)
        if EasyAH.key(self.records, record) then
            return true
        end
    end,
...
function M.new(parent, rows, columns)
    ...
    thumbTex:SetTexture(EasyAH.color.content.background())
    ...
    for i, column in ipairs(rt.columns) do
        ...
        text:SetTextColor(EasyAH.color.label.enabled())
