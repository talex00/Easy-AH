...
--		aux_background:SetTexture(EasyAH.color.content.background())
--		aux_background:SetAllPoints(DropDownList1Backdrop)
--		blizzard_backdrop = DropDownList1Backdrop:GetBackdrop()
--		EasyAH.hook('ToggleDropDownMenu', function(...)
--			T.temp(arg)
--			local ret = T.temp-T.list(EasyAH.orig.ToggleDropDownMenu(unpack(arg)))
--			local dropdown = _G[arg[4] or ''] or this:GetParent()
--			if strfind(dropdown:GetName() or '', '^EasyAH.frame%d+$') then
...
function M.checkbutton(parent, text_height)
    ...
    button.state = false
    button:SetBackdropColor(EasyAH.color.state.disabled())
    function button:SetChecked(state)
        if state then
            self:SetBackdropColor(EasyAH.color.state.enabled())
            self.state = true
        else
            self:SetBackdropColor(EasyAH.color.state.disabled())
            self.state = false
        end
    end
...
do
	...
	function mt.__index:create_tab(text)
		...
		dock:SetTexture(EasyAH.color.panel.background())
		tab.dock = dock
	...
	function mt.__index:update()
		for _, tab in self._tabs do
			if tab.group._selected == tab.id then
				tab.text:SetTextColor(EasyAH.color.label.enabled())
				tab:Disable()
				tab:SetBackdropColor(EasyAH.color.panel.background())
				tab.dock:Show()
				tab:SetHeight(29)
			else
				tab.text:SetTextColor(EasyAH.color.text.enabled())
				tab:Enable()
				tab:SetBackdropColor(EasyAH.color.content.background())
				tab.dock:Hide()
				tab:SetHeight(24)
			end
		end
...
function M.editbox(parent)
    ...
    editbox:SetScript('OnEditFocusGained', function()
	    ...
	    this.overlay:Hide()
	    this:SetTextColor(EasyAH.color.text.enabled())
	    this.focused = true
	    this:HighlightText()
        ...
        editbox:SetScript('OnMouseDown', function()
	        if arg1 == 'RightButton' then
	            ...
	            if GetTime() - last_click.t < .5 and x == last_click.x and y == last_click.y then
	                EasyAH.thread(function() editbox:HighlightText() end)
	            end
	            T.wipe(last_click)
    ...
    overlay:SetTextColor(EasyAH.color.text.enabled())
...
do
	...
	function M.status_bar(parent)
	    ...
	    do
	        ...
	        text:SetTextColor(EasyAH.color.text.enabled())
...
function M.horizontal_line(parent, y_offset, inverted_color)
    ...
    if inverted_color then
        texture:SetTexture(EasyAH.color.panel.background())
    else
        texture:SetTexture(EasyAH.color.content.background())
    end
    return texture
end
...
function M.slider(parent)
    ...
    thumb_texture:SetTexture(EasyAH.color.content.background())
    ...
    -- Single anchor only (no right-side point): the label sizes to its own
    -- text and never word-wraps/clips, regardless of how long the caption is.
    ...
    -- Wide/small enough to fit decimal coefficients (e.g. 0.94) and large
    -- whole numbers (e.g. 10000) without the client clipping the text.
    set_size(editbox, 60, 18)
...
function M.percentage_historical(pct, bid)
	...
	if bid then
		return EasyAH.color.gray(text)
	elseif pct < 50 then
		return EasyAH.color.blue(text)
	elseif pct < 80 then
		return EasyAH.color.green(text)
	elseif pct < 110 then
		return EasyAH.color.yellow(text)
	elseif pct < 135 then
		return EasyAH.color.orange(text)
	else
		return EasyAH.color.red(text)
	end
end