...
do
            ...
            function(index)
                ...
                if record.buyout_price > 0 then
                    buyout_button:SetScript('OnClick', function()
                        if scan_util.test(record, index) and listing:ContainsRecord(record) then
                            EasyAH.place_bid('bidder', index, record.buyout_price, function() listing:RemoveAuctionRecord(record) end)
                        end
                    end)
    ...
    function on_update()
        ...
        elseif state == FOUND and not scan_util.test(selection.record, found_index) then
            buyout_button:Disable()
            bid_button:Disable()
            if not EasyAH.bid_in_progress() then state = IDLE end
        end
    end
end