...
function EasyAH.handle.LOAD()
	recent_searches, favorite_searches = EasyAH.realm_data.recent_searches, EasyAH.realm_data.favorite_searches
end
...
function update_search_listings()
	...
	for i = 1, getn(favorite_searches) do
		...
		tinsert(favorite_search_rows, T.map(
			'cols', T.list(T.map('value', search.auto_buy and EasyAH.color.red'X' or search.auto_bid and EasyAH.color.red'Y' or ''), T.map('value', name)),
			'search', search,
			'index', i
		))
	end
	...
	for _, search in favorite_searches do
		if search.auto_buy then
			...
			if queries then
				...
				EasyAH.print('Invalid auto buy filter:', error)
			end
		end
	end
	return function(record)
		return EasyAH.any(validators, function(validator) return validator(record) end)
	end
end
	...
	for _, search in favorite_searches do
		if search.auto_bid then
			...
			if queries then
				...
				EasyAH.print('Invalid auto bid filter:', error)
			end
		end
	end
	return function(record)
...
function add_favorite(filter_string)
	...
	if queries then
		tinsert(favorite_searches, 1, T.map(
			'filter_string', filter_string,
			'prettified', EasyAH.join(EasyAH.map(queries, function(query) return query.prettified end), ';')
		))
		update_search_listings()
	else
...
function enable_auto_buy(search)
	...
	if queries then
		if getn(queries) > 1 then
			EasyAH.print('Error: Auto Buy does not support multi-queries')
		elseif EasyAH.size(queries[1].blizzard_query) > 0 and not filter_util.parse_filter_string(search.filter_string).blizzard.exact then
			EasyAH.print('Error: Auto Buy does not support Blizzard filters')
		else
...
function enable_auto_bid(search)
	...
	if queries then
		if getn(queries) > 1 then
			EasyAH.print('Error: Auto Bid does not support multi-queries')
		elseif EasyAH.size(queries[1].blizzard_query) > 0 and not filter_util.parse_filter_string(search.filter_string).blizzard.exact then
			EasyAH.print('Error: Auto Bid does not support Blizzard filters')
		else
