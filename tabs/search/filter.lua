...
function valid_level(str)
	...
	return level and EasyAH.bounded(1, 60, level)
end
...
function update_form()
	...
	if EasyAH.any(T.temp-T.list('min_level', 'max_level', 'usable', 'class', 'subclass', 'slot', 'quality'), function(key) return blizzard_query[key] end) then
...
function get_filter_builder_query()
	...
	if not EasyAH.index(filter_util.parse_filter_string(name), 'blizzard', 'name') then
...
function import_filter_string()
	local filter, error = filter_util.parse_filter_string(EasyAH.select(3, strfind(search_box:GetText(), '^([^;]*)')))
	if filter or EasyAH.print(error) then
		set_form(filter)
	end
	update_form()
end
...
-- add_component keeps working so importing an existing saved/favorite search
-- that happens to contain an advanced predicate (from before this panel was
-- removed) still preserves it in the exported filter string, even though
-- there's no UI left to add new ones by hand.
