...
CreateFrame('GameTooltip', 'EasyAHTooltip', nil, 'GameTooltipTemplate')
EasyAHTooltip:SetScript('OnTooltipAddMoney', function()
	this.money = arg1
end)
...
function M.auction(index, query_type)
	...
	if link then
        ...
        return T.map(
            ...
            'itemstring', item_info.itemstring,
            'item_key', item_id .. ':' .. suffix_id,
            'search_signature', EasyAH.join(T.temp-T.list(item_id, suffix_id, enchant_id, start_price, buyout_price, bid_price, aux_quantity, duration, query_type == 'owner' and high_bidder or (high_bidder and 1 or 0), EasyAH.account_data.ignore_owner and (is_player(owner) and 0 or 1) or (owner or '?')), ':'),
            'sniping_signature', EasyAH.join(T.temp-T.list(item_id, suffix_id, enchant_id, start_price, buyout_price, aux_quantity, EasyAH.account_data.ignore_owner and (is_player(owner) and 0 or 1) or (owner or '?')), ':'),
...
function M.tooltip_match(entry, tooltip)
    return EasyAH.any(tooltip, function(line)
...
function M.display_name(item_id, no_brackets, no_color)
    ...
    if item_info then
        ...
        if not no_color then
            name = EasyAH.select(4, GetItemQualityColor(item_info.quality)) .. name .. FONT_COLOR_CODE_CLOSE
        end
        return name
    end
end
...
function M.tooltip(setter, arg1, arg2)
    EasyAHTooltip:SetOwner(UIParent, 'ANCHOR_NONE')
    EasyAHTooltip.money = 0
    if setter == 'auction' then
	    EasyAHTooltip:SetAuctionItem(arg1, arg2)
    elseif setter == 'bag' then
	    EasyAHTooltip:SetBagItem(arg1, arg2)
    elseif setter == 'inventory' then
	    EasyAHTooltip:SetInventoryItem(arg1, arg2)
    elseif setter == 'link' then
	    EasyAHTooltip:SetHyperlink(arg1)
    end
    local tooltip = T.acquire()
    for i = 1, EasyAHTooltip:NumLines() do
        tinsert(tooltip, T.map(
            'left_text', _G['EasyAHTooltipTextLeft' .. i]:GetText(),
            'left_color', T.list(_G['EasyAHTooltipTextLeft' .. i]:GetTextColor()),
            'right_text', _G['EasyAHTooltipTextRight' .. i]:IsVisible() and _G['EasyAHTooltipTextRight' .. i]:GetText(),
            'right_color', T.list(_G['EasyAHTooltipTextRight' .. i]:GetTextColor())
        ))
    end
    return tooltip, EasyAHTooltip.money
end
