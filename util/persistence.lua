module 'EasyAH.util.persistence'
...
function read_list(schema, str)
    ...
    return EasyAH.map(EasyAH.split(str, separator), function(part)
...
function write_list(schema, list)
    local separator = schema[2]
    local element_type = schema[3]
    local parts = EasyAH.map(T.temp-EasyAH.copy(list), function(element)
        return write(element_type, element)
    end)
    return EasyAH.join(parts, separator)
end

function read_tuple(schema, str)
    local separator = schema[2]
    local tuple = T.acquire()
    local parts = T.temp-EasyAH.split(str, separator)
