    ...
    return {
        ...
        validator = function()
            return function(auction_record)
                return EasyAH.any(auction_record.tooltip, function(entry)
...
function operator(str)
    ...
    for name in T.temp-T.set('and', 'or') do
	    local arity = EasyAH.select(3, strfind(str, '^' .. name .. '(%d*)$'))
	    if arity then
...
do
	local mt = {
		__call = function(self, str, i)
			...
			local number = tonumber(EasyAH.select(3, strfind(str, '^(%d+)$')))
			if number then
			...
			for _, parser in T.temp-T.list(
				...
				T.temp-T.list('subclass', T.vararg-function(arg) return info.item_subclass_index(EasyAH.index(self.class, 2) or 0, unpack(arg)) end),
				T.temp-T.list('slot', T.vararg-function(arg) return info.item_slot_index(EasyAH.index(self.class, 2) == 2 and 2 or 0, EasyAH.index(self.subclass, 2) or 0, unpack(arg)) end),
				T.temp-T.list('quality', info.item_quality_index)
			) do
				if not self[parser[1]] then
					tinsert(parser, str)
					local index, label = parser[2](EasyAH.select(3, unpack(parser)))
					if index then
...
function parse_parameter(input_type, str)
    ...
    elseif type(input_type) == 'table' then
        return EasyAH.key(input_type, str)
    end
end
...
function M.parse_filter_string(str)
    ...
    local parts = str and EasyAH.map(EasyAH.split(str, '/'), function(part) return strlower(EasyAH.trim(part)) end) or T.acquire()
            ...
            if input_type ~= '' then
                if not parts[i + 1] or not parse_parameter(input_type, parts[i + 1]) then
                    if parts[i] == 'item' then
                        return nil, 'Invalid item name', EasyAH.account_data.auctionable_items
                    elseif type(input_type) == 'table' then
...
function M.queries(filter_string)
    local parts = EasyAH.split(filter_string, ';')
...
function suggestions(filter)
    ...
    if filter.blizzard.name and EasyAH.size(filter.blizzard) == 1 then tinsert(suggestions, 'exact') end
    ...
    if not filter.blizzard.subclass then
        for _, subclass in ipairs(T.temp-T.list(GetAuctionItemSubClasses(EasyAH.index(filter.blizzard.class, 2) or 0))) do
    ...
    if not filter.blizzard.slot then
        for _, invtype in ipairs(T.temp-T.list(GetAuctionInvTypes(EasyAH.index(filter.blizzard.class, 2) == 2 and 2 or 0, EasyAH.index(filter.blizzard.subclass, 2) or 0))) do
    ...
    if getn(filter.components) == 0 then
	    for _, name in ipairs(EasyAH.account_data.auctionable_items) do
	    ...
	    if component[1] == 'blizzard' then
		    if component[2] == 'name' then
			    if filter.blizzard.exact then
			        prettified.append(info.display_name(info.item_id(component[3])) or EasyAH.color.orange('[' .. component[3] .. ']'))
			    elseif component[3] ~= '' then
				    prettified.append(EasyAH.color.label.enabled(component[3]))
			    end
		    elseif component[2] ~= 'exact' then
			    prettified.append(EasyAH.color.orange(component[3]))
		    end
        elseif component[1] == 'operator' then
			prettified.append(EasyAH.color.orange(component[2] .. (component[2] ~= 'not' and tonumber(component[3]) or '')))
        elseif component[1] == 'filter' then
            if i == 1 or component[2] ~= 'tooltip' then
                prettified.append(EasyAH.color.orange(component[2]))
            end
            local parameter = component[3]
            if parameter then
	            if component[2] == 'item' then
		            prettified.append(info.display_name(info.item_id(parameter)) or EasyAH.color.label.enabled('[' .. parameter .. ']'))
	            else
		            if filters[component[2]].input_type == 'money' then
			            prettified.append(money.to_string(money.from_string(parameter), nil, true, EasyAH.color.label.enabled))
		            else
			            prettified.append(EasyAH.color.label.enabled(parameter))
		            end
	            end
            end
        end
...
function M.unquote(name)
	return EasyAH.select(3, strfind(name, '^<(.*)>$')) or name
end
...
function blizzard_query(filter)
    ...
    if filters.exact and item_info then
	    ...
	    for key in T.temp-T.set('min_level', 'max_level', 'class', 'subclass', 'slot', 'usable', 'quality') do
            query[key] = EasyAH.index(filters[key], 2)
	    end
    end
    return query
end
        ...
        for i = getn(filter.post), 1, -1 do
            ...
            if type == 'operator' then
                ...
                elseif name == 'and' then
                    tinsert(stack, EasyAH.all(args))
                elseif name == 'or' then
                    tinsert(stack, EasyAH.any(args))
                end
            elseif type == 'filter' then
                tinsert(stack, not not validators[i](record))
            end
        end
        return EasyAH.all(stack)
    end
end
