...
function M.to_string2(money, exact, color)
	...
	if not exact and money >= COPPER_PER_GOLD then
		money = EasyAH.round(money / COPPER_PER_SILVER) * COPPER_PER_SILVER
	end
...
function M.from_string(value)
	...
	local gold = tonumber(EasyAH.select(3, strfind(value, '(%d*%.?%d+)g')))
	local silver = tonumber(EasyAH.select(3, strfind(value, '(%d*%.?%d+)s')))
	local copper = tonumber(EasyAH.select(3, strfind(value, '(%d*%.?%d+)c')))
